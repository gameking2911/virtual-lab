...
----socket3
	[tmsh:getSubMesh(socket3[3])] = { IDtable={self.slceboard.ID,socket3[3]},  },
	[tmsh:getSubMesh(socket3[4])] = { IDtable={self.slceboard.ID,socket3[4]},  },
	[tmsh:getSubMesh(socket3[10])] = { IDtable={self.slceboard.ID,socket3[10]},  },
	[tmsh:getSubMesh(socket3[11])] = { IDtable={self.slceboard.ID,socket3[11]},  },
----socket4
...
-- ----socket3
	[tmsh:getSubMesh(socket3[1])] = { IDtable={self.slceboard.ID,socket3[1]},  },
	[tmsh:getSubMesh(socket3[2])] = { IDtable={self.slceboard.ID,socket3[2]},  },	
	[tmsh:getSubMesh(socket3[5])] = { IDtable={self.slceboard.ID,socket3[5]},  },
	[tmsh:getSubMesh(socket3[6])] = { IDtable={self.slceboard.ID,socket3[6]},  },	
	[tmsh:getSubMesh(socket3[8])] = { IDtable={self.slceboard.ID,socket3[8]},  },
	[tmsh:getSubMesh(socket3[9])] = { IDtable={self.slceboard.ID,socket3[9]},  },	
	[tmsh:getSubMesh(socket3[12])] = { IDtable={self.slceboard.ID,socket3[12]},  },
	[tmsh:getSubMesh(socket3[13])] = { IDtable={self.slceboard.ID,socket3[13]},  },
...
------初始化，将所用芯片的所有与非门的输入端口和输出端口连接起来，因为经过与非门，所以isDirectConnection置为false
	...
	for i=1,#socket3 do
		if self.outputIndex[tmsh][tmsh:getSubMesh(socket3[i])] then
			self.sl_tree:IDtable2Node( self.outputIndex[tmsh][tmsh:getSubMesh(socket3[i])].IDtable )
		end
		if self.inputIndex[tmsh][tmsh:getSubMesh(socket3[i])] then 
			self.sl_tree:IDtable2Node( self.inputIndex[tmsh][tmsh:getSubMesh(socket3[i])].IDtable )		
		end
	end
...
----时序逻辑树的重置
	...
	local socket3 = SLCE.slceboard.config.key.socket_3
...
----逻辑值置为nil
	...
	for i=1,#socket1 do
		...
		if SLCE.inputIndex[tmsh][tmsh:getSubMesh(socket3[i])] then
			SLCE.sl_tree:destroyConnection( SLCE.inputIndex[tmsh][tmsh:getSubMesh(socket3[i])].IDtable )
		end
	end
...
function SLCE.cc4011:place()
	Object.place(self)
	local socket3 = SLCE.slceboard.config.key.socket_3
	ChipInit { chip = self, load_or_delete = true, experiment = SLCE,main_socket = SLCE.slceboard,sub_socket = socket3}
end

function SLCE.cc4011:unplace()
	Object.unplace(self)
	local socket3 = SLCE.slceboard.config.key.socket_3
	ChipInit { chip = self, load_or_delete = false, experiment = SLCE,main_socket = SLCE.slceboard,sub_socket = socket3 }
end
...
function ui.model.R.r1.click(self)
	linkfinished()
	exp.sm:stateTransition("ClickC/Ibtn") 
	exp.choice  = _Mesh.new('cc4011.skn')
	exp.choice.class = classID.cc4011
	exp.choice:enumMesh( '', true, function(m)  m.blender = twinkle end )
	exp:placed(exp.choice.class)
end
